...
authedTest.describe('Cron Jobs — API', () => {
  ...
  authedTest('POST /api/crons — valid 5-field cron, cleanup', async () => {
    const res = await api.post('/api/crons', { name: 'e2e-cron', cron_expr: '0 * * * *', provider: null })
    expect(res.status).toBe(201)
    const body = await res.json() as { id: number; cron_expr: string }
    expect(body.cron_expr).toBe('0 * * * *')
    await api.delete(`/api/crons/${body.id}`)
  })

  authedTest('POST /api/crons — invalid expression → 422', async () => {
    expect((await api.post('/api/crons', { name: 'bad', cron_expr: 'not-a-cron', provider: null })).status).toBe(422)
  })

  authedTest('PUT /api/crons/:id — update schedule', async () => {
    const created = await (await api.post('/api/crons', { name: 'e2e-upd', cron_expr: '0 * * * *', provider: null })).json() as { id: number }
    const updated = await (await api.put(`/api/crons/${created.id}`, { name: 'e2e-upd', cron_expr: '0 0 * * *', provider: null })).json() as { cron_expr: string }
  ...
  authedTest('PATCH /api/crons/:id/toggle — flips is_active', async () => {
    const created = await (await api.post('/api/crons', { name: 'e2e-tog', cron_expr: '0 * * * *', provider: null })).json() as { id: number; is_active: boolean }
  ...
  authedTest('POST /api/crons/:id/run-now — 200', async () => {
    const created = await (await api.post('/api/crons', { name: 'e2e-run', cron_expr: '0 * * * *', provider: null })).json() as { id: number }
