...
export function DotPattern({
  ...
  const buildGrid = useCallback(() => {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        dots.push({ x: offsetX + col * cellSize, y: offsetY + row * cellSize, baseOpacity: 0.25 + Math.random() * 0.15 })
      }
    }
  ...
  const draw = useCallback(() => {
    ...
    for (const dot of dotsRef.current) {
      ...
      const waveOpacity = dot.baseOpacity + wave * 0.12
  ...
  return (
    <div ref={containerRef} className={cn("fixed inset-0 overflow-hidden pointer-events-none", className)}>
